        ...
        <div className="space-y-3">
          ...
          {isPro && (
            <button
              onClick={handleManageSubscription}
              disabled={portalLoading}
              className="w-full flex items-center gap-3 px-5 py-4 rounded-2xl border border-amber-500/20 bg-card text-sm text-amber-400 hover:text-amber-300 hover:bg-amber-500/5 transition-all duration-200 disabled:opacity-50"
            >
              ⚠️ {portalLoading ? "Loading..." : "Cancel subscription"}
            </button>
          )}
          <button
            onClick={signOut}
